...
class SmoothValue:
    ...
    @classmethod
    def addCallback(cls, trial, page):
        if trial == 'success':
            info = f'Successfully fetch!! | page {page} | time: {SmoothValue.timeRecorder()}'
            cls.success += 1
        else:
            info = f'Fail to deleted file | page {page} | time: {SmoothValue.timeRecorder()}'
            cls.fail += 1
        print(info)
        SmoothValue.log(info)
    ...
    @classmethod
    def error(cls):
        raise Exception('You should fetch first, try PTT().fetch')